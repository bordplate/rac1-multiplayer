...
void ServerListView::on_load() {
    ...
    username_label_ = new TextElement(120, 390, label.c_str(), Movie);

    this->add_element(username_label_);
    this->add_element(new TextElement(380, 390, "\x12 Back", Movie));
    this->add_element(new TextElement(380, 370, "\x11 Direct IP", Movie));
    ...
    if (servers_.size() > 0) {
        for (int i = 0; i < servers_.size(); i++) {
            ...
            if (server) {
                ...
                TextElement* element = new TextElement(250, y, "", Movie);
