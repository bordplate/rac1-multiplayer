...
LD_LOADER_TEMPLATE = \
'''
.sys_prx_load_module = {SYS_PRX_LOAD_MODULE_ADDR}; 
.sys_prx_start_module = {SYS_PRX_START_MODULE_ADDR};

SECTIONS 
{
    . = {LOADER_INJECT_ADDR};
    .text.inject : { *(.text.inject) }
...
def main():
    ...
    with open( os.path.join( args.elf_out_dir, "shk_elf_loader.gen.ld" ), "w" ) as f:
        src = fillTemplate( LD_LOADER_TEMPLATE, 
            SYS_PRX_LOAD_MODULE_ADDR=hex( args.sys_prx_load_module_addr ),
            SYS_PRX_START_MODULE_ADDR=hex( args.sys_prx_start_module_addr ),
            LOADER_INJECT_ADDR=hex( args.loader_inject_addr ),
            LOADER_ADDR=hex( args.loader_text_range[0] ))
        f.write( src + "\n" )
