...
patch: binary
	$(PYTHON) "$(GENERATEPATCHFILE)" --input_file {HOOK_INPUTS} --input "$(TMP_DIR)/.text.shk_elf_shared.bin" "$(TMP_DIR)/.data.shk_elf_shared.bin" --address_file {HOOK_ADDRESSES} --address $(SHARED_TEXT_ADDRESS) $(SHARED_DATA_ADDRESS) --output "$(TMP_DIR)/patch.txt" --append
'''
...
def main():
    ...
    with (
        open(os.path.join(args.elf_out_dir, "shk_elf.gen.mk"), "w") as f,
        open(os.path.join(args.elf_out_dir, "binary_hooks.txt"), "w") as binary_hooks,
        open(os.path.join(args.elf_out_dir, "binary_addresses.txt"), "w") as addresses
    ):
        f.write(f'# generated by {get_script_name()}\n')
        ...
        for hook in hooks:
            ...
            binary_hooks.write(f"{args.elf_out_dir}/.text.shk_elf_patch_{hook.name}.bin\n")

        for patch in patches:
            binary_hooks_src += "\n" + fill_template(MK_INJECT_HOOK_OUTPUT_TEMPLATE.strip("\n"), PREFIX="shk_game_patch", HOOK=f"0x{patch.addr:X}")
            binary_hooks.write(f"{args.elf_out_dir}/.text.shk_game_patch_0x{patch.addr:X}.bin\n")
        ...
        for hook in hooks:
            addresses.write(f"{hex(hook.addr)}\n")

        for patch in patches:
            addresses.write(f"{hex(patch.addr)}\n")
        ...
        src = fill_template(MK_INJECT_TEMPLATE,
                            ...
                            HOOK_INPUTS=os.path.join(args.elf_out_dir, "binary_hooks.txt").replace("\\", "/"),
                            HOOK_ADDRESSES=os.path.join(args.elf_out_dir, "binary_addresses.txt").replace("\\", "/"),
